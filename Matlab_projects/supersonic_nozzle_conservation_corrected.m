...
for tt = 1:length(nt) 
 % Storing Initial Values 
    ...
    for mx = 2:n-1 
    % Source term J2
    J2(mx) = (1/gamma)*rho(mx)*t(mx)*((a(mx)-a(mx-1))/dx);
    % Correctors Step derivative  
    du1dt_c(mx) = -(F1(mx)-F1(mx-1))./dx ;
    du2dt_c(mx) = -(F2(mx)-F2(mx-1))./dx + J2(mx) ;
    du3dt_c(mx) = -(F3(mx)-F3(mx-1))./dx ;
...
V_throat(tt) = V(16);
t_throat(tt) = t(16);
rho_throat(tt) = rho(16);
p_throat(tt)  = pressure(16);
mach_throat(tt) = mach_number(16);
mass_f_r_throat(tt) = mass_flow_rate(16);
